...
def main(args):
  ...
  output.to_csv(f'./prediction/{date}_{ts}_{config.model_name.split("/")[0]}-{config.model_name.split("/")[1]}-{config.max_epoch}e.csv', index=False) # 최종적으로 완성된 예측한 라벨 csv 파일 형태로 저장.
  #### 필수!! ##############################################
  print('---- Finish! ----')
